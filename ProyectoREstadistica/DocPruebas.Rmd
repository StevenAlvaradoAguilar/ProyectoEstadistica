...
Es importante que el usuario tome en consideración que la creación de documentos Rmarkdown  implica una combinación de texto plano y bloques de código (chunk), se manera que lo que quede fuera de estos bloques R no lo ejecuta.
...
Para instalar una paquete en R se puede hacer de dos maneras:
...
Si se desea instalar varios paquete de manera simultánea, solo que se debe hacer es la primer argumento de función `install.packages` un vector de caracteres:
...
# Solitando ayuda en R

Para pedir ayuda a R se hace mediante el simbólo signo de pregunta antes del comando respectivo, por ejemplo para pedir ayuda para el comando *matrix* se hace como sigue: **?matrix**
...
Primermanete veamos como cargar una base de estudiantes existente en R:

```{r}
data("airquality")

# se almacena en una variables para uso posterior
...
Con lo anterior, la base de estudiantes se pudo hacer leído de la siguiente manera:

```{r}
estudiantes <- read.csv2("Datos/EjemploEstudiantes.csv", header = T, row.names = 1)
```

La siguiente instrucción del paquete `readr` permite cargar bases de estudiantes de una manera rápida, pero tiene el problema de que no reconoce `row names` en caso de que sea de interés, en su defecto, lee esta columna como un tipo caracter y le asigna un nombre a esta *X1*. Al igual que en el caso anterior, existen dos versiones `read_csv`, para separador de columna `,` y `read_csv2`, para separador de columna `;`.
...
Si sw desea, se puede redondear el resultado:
...
Las siguientes instrucciones dan error:
...
En el lenguaje de `R`, los elementos u objetos que se vayan definiendo, ya se por nosotros mismos o bien como resultado del programa, pueden y deben ser distinguidos para su uso correcto. Concretamente, vamos a hablar de:
...
Los siguientes vectores siguen una distribución de probabilidad específia:
```{r}
y <-  rnorm(50)
y
```
...
Note que `R` ordena (Levels) los valores de la variable factor por orden alfabético, pero si no se desea seguir ese orden, sino por ejemplo, el orden en el que los países están ordenados geográficamente en la dirección de sur a norte, se le puede indicar a `R` con la siguiente instrucción:
...
Los siguientes ejemplos debe ilustrar esto:
